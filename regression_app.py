...
st.write('This is a web app to predict the arrival delay time of the trains (in minutes).')
st.write('Please adjust the value of each feature in the sidebar, and then click on the Predict button at the bottom to see the prediction of the model.')
...
model = load_model(base_dir + 'model/regression_lightgbm_short_term')                           # Using Pycaret
...
if st.button('Predict'):
    ...
    st.header(' Based on feature values, the trip will arrive ' + str(round(prediction[1],1)) + ' minutes '+ str(prediction[0]))      #   Using pycaret
