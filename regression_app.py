...
model = load_model(base_dir +
                   'model/regression_lightgbm_short_term')  # Using Pycaret

# Input features

teu_count = st.sidebar.slider(label='TEU count',
                              min_value=0,
                              max_value=100,
                              value=60,
                              step=1)

train_length = st.sidebar.slider(label='Train length [m]',
                                 min_value=100,
                                 max_value=720,
                                 value=550,
                                 step=10)

train_weight = st.sidebar.slider(label='Train weight [t]',
                                 min_value=290,
                                 max_value=2300,
                                 value=1200,
                                 step=10)

wagon_count = st.sidebar.slider(label='Number of wagons',
                                min_value=6,
                                max_value=35,
                                value=15,
                                step=1)
...
departure_delay = st.sidebar.slider(label='Departure delay [min]',
                                    min_value=-250,
                                    max_value=950,
                                    value=30,
                                    step=5)
...
st.table(
    features_df.rename(
        columns={
            ...
            'Distance between control stations [km]',
...
if st.button('Predict'):
    ...
    st.header(' Based on feature values, the trip will arrive ' +
              str(round(prediction[1], 1)) + ' minutes ' +
              str(prediction[0]))  #   Using pycaret
